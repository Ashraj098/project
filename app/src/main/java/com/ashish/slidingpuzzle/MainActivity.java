...
public class MainActivity extends AppCompatActivity {
    ...
    private boolean defaultAdapter; //A boolean to track which Data, i.e. default photos or camera photos are being shown in recyclerView
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final int[] drawableInts = {
                R.drawable.dora,R.drawable.doraemon,R.drawable.gian,R.drawable.nobita,R.drawable.shin_chan,
                R.drawable.shizuka,R.drawable.suniyo,R.drawable.tokyo
        };
        getSavedPhotos();
        mRecyclerView=findViewById(R.id.pictureRecyclerView); //getting the Scrolling view for images
        mRecyclerView.setVisibility(View.INVISIBLE);
        mRecyclerView.setHasFixedSize(true);

        int orientation = getResources().getConfiguration().orientation; //Getting the orientation settings
        // use layout manager - horizontal orientation = 0, vertical = 1
        RecyclerView.LayoutManager layoutManager; //Creating an instance of LayoutManager
        if (orientation == Configuration.ORIENTATION_LANDSCAPE) {
            layoutManager = new LinearLayoutManager(this, RecyclerView.HORIZONTAL, false);
        } else {
            layoutManager = new LinearLayoutManager(this, RecyclerView.VERTICAL, false);
        }
        mRecyclerView.setLayoutManager(layoutManager); //Setting the Layout for recycler based on orientation

        // set adapter to default use default image dataset
        final ImageRecyclerAdapter testAdapter = new ImageRecyclerAdapter(drawableInts); //ImageRecyclerAdapter is an Adapter that we have created
        //for passing the Images.
        mRecyclerView.setAdapter(testAdapter); //Setting testadapter as the adapter for our RecyclerView
        defaultAdapter = true;
        ...
        final ImageRecyclerAdapter photoAdapter = new ImageRecyclerAdapter(savedPhotos); //Adapter for Passing the images taken from camera in the app
        ...
        adapterButton.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                ...
                if (isChecked) {  // recycler displaying default images -> change to app photo gallery
                    ...
                    mRecyclerView.swapAdapter(photoAdapter, true); //Swap the default photo adapter with Camera Photo adapter
        ...
        loadButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                int selectedImage = testAdapter.getSelection(); //selectedImage contains the array index of selected image
                ...
                if (selectedImage == -1) {
                    ...
                    if (defaultAdapter) {  // selection is from default images
                        ...
                        gameIntent.putExtra("puzzleNum", selectedImage);//Put the index of currently selected image in puzzleNum
